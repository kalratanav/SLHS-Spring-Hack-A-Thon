...
var DayPlanner = (function() {
  ...
  // *** ITEMS ***
    ...
    // item name
      ...
      var getSubjectDiv = function(item) {
        return item.querySelector(".subject");
      };

      var getSubjectInput = function(item) {
        return getSubjectDiv(item).getElementsByTagName("input")[0];
      };

      var getSubject = function(item) {
        return getSubjectInput(item).value.trim();
      };

      var setSubject = function(item, name) {
        if (name.length > maxItemNameLength) {
          name = name.substring(0, maxItemNameLength);
        }
        getSubjectInput(item).value = name;
      };
  ...
  // *** ITEMS MENU ***
    ...
    var showMenu = function(item) {
      ...
      getSubjectInput(item).readOnly = false;
    ...
    var hideMenu = function() {
      ...
      getSubjectInput(openedItem).readOnly = true;
